...
cursor.execute('''SELECT x.* FROM public.jobs x
                WHERE searchterm = 'Dentistry'
                ORDER BY RANDOM()
                limit 1000''')
records = cursor.fetchall()

dbtime = timeit.default_timer()


print("Fetch time : ", dbtime - starttime)

# Zip it all up into one loooong string
indeedLink = records[0][14]
print(indeedLink)
...
results = kw_model.extract_keywords(descStr, keyphrase_ngram_range=(1,1), top_n=10, stop_words=stopwords)
# print(results)

keywords = list(zip(*results))[0]
print(keywords)

print("Processing time : ", timeit.default_timer() - dbtime)
